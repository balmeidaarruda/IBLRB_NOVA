...
#Region "BUSCA DADOS"
    ...
    Public Function getListaCampos(ByVal _descricao As String) As List(Of Campo)
        Dim sql As New StringBuilder
        Dim listaCampos As New List(Of Campo)

        If _descricao = String.Empty Then
            sql.AppendLine("Select * from IBLR_CAMPOS")
        Else
            sql.AppendLine("Select * from IBLR_CAMPOS WHERE NOME LIKE '%" & _descricao & "%'")
        End If
    ...
    Public Function getCongregacaoUltimaInserida() As String
        Dim sql As New StringBuilder
        sql.AppendLine("SELECT max(Id) as ID FROM IBLR_CONGREGACOES")
        Dim conn As SqlConnection = ConexaoDB.RetornaConexaoIBLR
        Dim dr As SqlDataReader = ConexaoDB.ExecuteSelectSQL(conn, sql.ToString)
        Dim idMax As String = ""
        If dr.Read Then
